...
-- On reprend les nœuds de la première grille d'échantillonnage, Incref 0
...
SELECT UpdateGeometrySRID('inv_prod_new', 'noeud', 'geom', '2154');
...
WITH croise AS (
    ...
    INNER JOIN sig_ign.deps_2002 d ON st_intersects (st_transform(d.geom,2154),n.geom)
    WHERE c.millesime = 2025
    AND e.type_ech = 'IFN' AND e.phase_stat = 1 AND e.type_ue = 'P' AND e.passage = 1
)
UPDATE noeud_ech ne
...
-- Pour les nœuds sans région forestière, on récupère la région la plus proche
WITH empty_nodes AS (
...
CREATE TABLE public.alti_noeuds (
    ...
    id_noeud INT4,
    zp numeric(7, 2),
    CONSTRAINT alti_noeuds_pkey PRIMARY KEY (id_grille, id_noeud)
);

\COPY public.alti_noeuds FROM '/home/lhaugomat/Documents/GITLAB/production/Campagne_2025/donnees/altitude_noeud_grille1_incref0.csv' WITH CSV DELIMITER ';' NULL AS '' HEADER

UPDATE public.alti_noeuds
SET zp = 0
WHERE zp < -500;
...
UPDATE public.pts_new SET xl = 890347,  yl = 2144608 WHERE npp = '25-01-273-1-133T';
UPDATE public.pts_new SET xl = 1004077,  yl = 1866475 WHERE npp = '25-06-335-1-205T';
UPDATE public.pts_new SET xl = 849038,  yl = 1801594 WHERE npp = '25-13-295-1-240T';
UPDATE public.pts_new SET xl = 327880,  yl = 2118210 WHERE npp = '25-17-107-1-197T';
UPDATE public.pts_new SET xl = 125597,  yl = 2319570 WHERE npp = '25-29-026-1-157T';
UPDATE public.pts_new SET xl = 1181052,  yl = 1617497 WHERE npp = '25-2A-369-1-262T';
...
CREATE TABLE public.alti_points (
    npp TEXT, 
    zp numeric(7, 2),
    CONSTRAINT alti_points_pkey PRIMARY KEY (npp)
);

\COPY public.alti_points FROM '/home/lhaugomat/Documents/GITLAB/production/Campagne_2025/donnees/altitude_points_2025.csv' WITH CSV DELIMITER ';' NULL AS '' HEADER
...
SELECT UpdateGeometrySRID('sig_inventaire', 'regn', 'geom', 2154);
...
\COPY (SELECT npp, nph, poi, id_noeud, ROUND(xl::NUMERIC, 0) AS xl, ROUND(yl::NUMERIC, 0) AS yl, 2025 AS campagne, poids, zp, regn, ser_86, ser_alluv, dep, commune, geom93 AS geom FROM pts_new ORDER BY npp) TO '/home/lhaugomat/Documents/GITLAB/production/Campagne_2025/donnees/pts_20.csv' WITH CSV HEADER DELIMITER ';' NULL AS ''
