...
SELECT * FROM metaifn.ajoutchamp('RU_AF', 'G3ECOLOGIE', 'INV_EXP_NM', FALSE, 0, 19, 'varchar', 1);
SELECT * FROM metaifn.ajoutchamp('RU_AF', 'P3ECOLOGIE', 'INV_EXP_NM', FALSE, 0, 19, 'varchar', 1);

-- Création des colonnes en base
	--> en base d'exploitation
ALTER TABLE inv_exp_nm.g3ecologie ADD COLUMN ru_af FLOAT8;
ALTER TABLE inv_exp_nm.p3ecologie ADD COLUMN ru_af FLOAT8;
	--> en base de production
ALTER FOREIGN TABLE inv_exp_nm.g3ecologie ADD COLUMN ru_af FLOAT8;
ALTER FOREIGN TABLE inv_exp_nm.p3ecologie ADD COLUMN ru_af FLOAT8;
...
--------------------------------------------------------------------------------
-- Contrôle
SELECT incref, COUNT(ru_af), sum(ru_af)
FROM inv_exp_nm.g3ecologie
GROUP BY incref
ORDER BY incref DESC;

SELECT incref, COUNT(ru_af), sum(ru_af)
FROM inv_exp_nm.p3ecologie
GROUP BY incref
ORDER BY incref DESC;

SELECT incref, COUNT(u_ru_af), sum(u_ru_af)
...
--------------------------------------------------------------------
-- Suppression de la donnée U
	--> en base d'exploitation
ALTER TABLE inv_exp_nm.g3ecologie DROP COLUMN u_ru_af;
ALTER TABLE inv_exp_nm.p3ecologie DROP COLUMN u_ru_af;
	--> en base de production
ALTER FOREIGN TABLE inv_exp_nm.g3ecologie DROP COLUMN u_ru_af;
ALTER FOREIGN TABLE inv_exp_nm.p3ecologie DROP COLUMN u_ru_af;
