...
FROM metaifn.abmode WHERE unite = 'QUAD'
ORDER BY absc, ord;
...
VALUES ('QUAD32', 'AUTRE', 'NOMINAL', $$Quadrats 32x32 km$$, $$Regroupement du découpage géographique des quadrats 16x16 km du DSF, en 32x32 km (4 quadrats en 1 regroupé).$$);

INSERT INTO metaifn.abmode (unite, mode, position, classe, etendue, libelle, definition)
SELECT 'QUAD32', "mode", row_number() over(), row_number() over(), 1, libelle, definition
FROM regroupe2
ORDER BY absc2, ord2;

INSERT INTO metaifn.abgroupe(gunite, gmode, unite, mode)
SELECT 'QUAD32' AS gunite, r."mode" AS gmode, 'QUAD' AS unite, d."mode"
FROM regroupe2 r
