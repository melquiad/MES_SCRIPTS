...
WITH new_arbres AS 
(
SELECT v2.annee AS campagne, p.idp, v2.id_ech, v2.npp, v2.id_point, a2.a--, COALESCE(fea.wac, pea.wac, fem.wac, pem.wac) AS w
FROM v_liste_points_lt2 v2
INNER JOIN arbre a2 USING (id_ech, id_point)
INNER JOIN point p USING (id_point)
INNER JOIN public.placette pla ON p.idp = pla.idp
--LEFT JOIN inv_exp_nm.g3arbre fea ON p.npp = fea.npp AND a2.a = fea.a
--LEFT JOIN inv_exp_nm.p3arbre pea ON p.npp = pea.npp AND a2.a = pea.a
--LEFT JOIN inv_exp_nm.g3morts fem ON p.npp = fem.npp AND a2.a = fem.a
--LEFT JOIN inv_exp_nm.p3morts pem ON p.npp = pem.npp AND a2.a = pem.a
WHERE NOT EXISTS (
			SELECT 1
...
SELECT DISTINCT na.campagne, na.idp, na.a--, na.w--, na.npp
FROM new_arbres na
ORDER BY idp, a;
