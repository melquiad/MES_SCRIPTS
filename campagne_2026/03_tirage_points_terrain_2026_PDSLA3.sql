...
-- CROISEMENT AVEC LES ZFOR ET RECALCUL DE ZTIR
DROP TABLE IF EXISTS noeuds0;

CREATE TEMPORARY TABLE noeuds0 AS 
SELECT ne.id_ech, ne.id_noeud, st_transform(n.geom, 27572) AS geom, NULL::char(1) AS zforifnd, NULL::char(1) AS zforifn
--SELECT ne.id_ech, ne.id_noeud, st_transform(n.geom, 932006) AS geom, null as zforifnd, NULL::char(1) AS zforifn
FROM noeud_ech ne
...
CREATE INDEX noeuds0_geom_gist ON noeuds0 USING GIST (geom);

UPDATE noeuds0 n
SET zforifnd = (vz.zforcode::int2)::char(1)
FROM sig_inventaire.vt55zfo2 vz
WHERE vz.geom && n.geom AND st_intersects(n.geom, vz.geom);

/*
SELECT id_noeud, st_astext(geom)
FROM noeuds0
WHERE zforifnd IS NULL; --0 point "hors territoire" en 2026
*/

UPDATE noeuds0
SET zforifnd = '2'
WHERE zforifnd IS NULL;
...
WHERE g.unite = 'ZFOR1'
AND g.gunite = 'ZFORG2'
AND g.mode = n.zforifnd;

WITH nds AS (
    SELECT id_ech, id_noeud, n0.zforifnd, n0.zforifn
    , CASE
        -- zone de maquis Corse
        WHEN depn IN ('2A', '2B') AND regn IN ('2A8', '2B7', '2B3') AND zp < 550 THEN '5'
        WHEN depn IN ('2A', '2B') AND regn NOT IN ('2A8', '2B7', '2B3') AND zp < 800 THEN '5'
        ...
        WHEN depn IN ('2A', '2B') AND NOT (regn NOT IN ('2A8', '2B7', '2B3') AND zp < 800) THEN '6'
        -- zone montagne
        WHEN n0.zforifn = '6' THEN '3'
        -- zone de garrigue ou maquis
        WHEN n0.zforifn = '4' THEN '2'
        -- zone de forêt homogène
        WHEN n0.zforifn = '1' THEN '1'
        -- zone des régions forestières du Sud-Est
        WHEN regn IN ('041', '042', '043', '044', '045', '046', '049', '051', '052', '053', '054', '055', '056', '057', '059', '061', '062', '063', '064', '067', '2A0', '2A8', '2A9', '2AS', '2B1', '2B2', '2B3', '2B4', '2B5', '2B6', '2B7'
                        , '841', '842', '845', '847', '263', '265', '266', '267', '268', '269', '072', '073', '074', '075', '076', '077', '303', '343', '485', '124', '126', '811', '812', '114', '116', '118', '664', '665', '666', '667', '668', '669', '092', '094', '097')
            THEN '4'
      ...
      END AS ztir
    FROM noeud_ech ne
    INNER JOIN noeuds0 n0 USING (id_ech, id_noeud)
)
UPDATE noeud_ech n
SET zforifnd = nds.zforifnd, zforifn = nds.zforifn, ztir = nds.ztir
FROM nds
WHERE n.id_ech = nds.id_ech AND n.id_noeud = nds.id_noeud;


/* VÉRIFIER QUE TOUTES LES ZONES DE TIRAGES SONT BIEN LÀ !!!
...
, CASE
    ...
    WHEN pt.csa IN ('6A', '7', '8', '9') THEN '0' --> ok
...
, CASE
    ...
    WHEN pt.csa IN ('6A', '7', '8', '9') THEN '2_CSA improductif'    
...
, st_x(st_transform(p.geom, 27572)) AS xl2
, st_y(st_transform(p.geom, 27572)) AS yl2
, st_x(p.geom) AS xl93
, st_y(p.geom) AS yl93
FROM pts_retour pt
...
SELECT ech_actuel, id_point, npp, round(st_x(geom)) as xl93, round(st_y(geom)) as yl93
FROM pts_retour p
INNER JOIN point pt USING (id_point)
WHERE tir5 = '1'
ORDER BY 1, 2;
...
\COPY public.declinaison2 FROM '/home/lhaugomat/Documents/ECHANGES/GIT/base-de-production/Campagne_2026/donnees/decli_revPI_2026.csv' WITH CSV DELIMITER ';' NULL AS ''
...
-- /!\ En 2026, il faut tirer les points passés à la forêt sur l'échantillon PI 2025 sur campagne 2021 dans les départements suivants :

-- ('03', '05', '08', '09', '10', '12', '15', '22', '27', '31', 
-- '32', '42', '43', '44', '46', '49', '50', '51', '52', '53', 
-- '54', '55', '57', '59', '63', '65', '72', '76', '81', '82', '85')
...
WHERE id_ech = 145; --AND dep NOT IN 
--('03', '05', '08', '09', '10', '12', '15', '22', '27', '31', 
'32', '42', '43', '44', '46', '49', '50', '51', '52', '53', 
'54', '55', '57', '59', '63', '65', '72', '76', '81', '82', '85')
...
FROM point_pi
INNER JOIN point_ech pe USING (id_ech, id_point)
WHERE id_ech = 111 AND dep NOT IN ('01', '02', '03', '04', '05', '08', '09', '10', '11', '12', '15', '17', '22', '24', '27', '29', '2A', '2B', '30', '31', '32', '33'
...
--> Version Cédric de la récupération des 2 échantillons
/*	        
...
SELECT 
  ...
  END AS dir_init
, count(*)
FROM v_liste_points_lt2 v                             -->idem avec lt1 et lt1_pi2 et lt2
INNER JOIN point p USING (id_point)
...
\COPY public.declinaison1 FROM '/home/lhaugomat/Documents/ECHANGES/GIT/base-de-production/Campagne_2026/donnees/decli_nouv_2026.csv' WITH CSV DELIMITER ';' NULL AS ''
...
, (144, 8, '3', 46, 3, NULL)--'{5, 8}')
, (144, 8, '4', 46, 3, NULL)--'{5, 8}')
, (144, 8, '5', 46, 4, NULL)--'{6, 9}')
, (144, 8, '6', 46, 2, NULL)--'{4, 7}')  --> 46 = 32 + 14 pour regrouper forêt et peupleraie,
, (144, 8, '0', 16, 3, NULL)             --> cf donnée Formation unité TMS dans abmode.
, (144, 8, '1', 16, 4, NULL)
, (144, 8, '2', 16, 4, NULL)
, (144, 8, '3', 16, 4, NULL)
, (144, 8, '4', 16, 4, NULL)
...
--    WHERE fp.flpi NOT IN ('0', 'A') --AND abs(fp.disti) <= 25
--    AND pp.pdsla = '3'
--    AND c.millesime = 2026
--)
...
, CASE
    ...
    CASE
        ...
        WHEN pp.pdsla = '3' THEN 960                                -- point dans SLA > 15m
        ELSE 0                                                      -- autre
    END
...
INNER JOIN point p USING (id_point)
--LEFT JOIN plhf l USING (id_point)
LEFT JOIN point_pi pp ON pp.id_ech = pepi.id_ech AND pp.id_point = pepi.id_point
...
--TABLE tirage ORDER BY id_ech DESC;

DROP TABLE IF EXISTS points_tir_final;
DROP TABLE public.points_tir_final;

CREATE TABLE public.points_tir_final AS
--CREATE TEMPORARY TABLE points_tir_final AS
SELECT DISTINCT ON (p.id_ech_ph2, p.id_point) p.id_ech_ph2 AS id_ech, p.id_point, p.formation, p.npp, p.pdsla
, CASE
    WHEN t.nvx_alleges IS NULL THEN power(2, t.niveau - 1)::REAL
    WHEN array_length(t.nvx_alleges, 1) = 1 THEN power(2.0, t.nvx_alleges[1]) / (power(2.0, t.nvx_alleges[1] - t.niveau + 1) - 1)
    WHEN array_length(t.nvx_alleges, 1) = 2 THEN power(2.0, t.nvx_alleges[2]) / (power(2.0, t.nvx_alleges[2] - t.niveau + 1) - power(2.0, t.nvx_alleges[2] - t.nvx_alleges[1]) - 1)
...
ALTER TABLE public.points_tir_final ADD CONSTRAINT points_tir_final_pkey PRIMARY KEY (id_ech, id_point);

--SELECT count(*) FROM points_tir_final;
...
LEFT JOIN sig_inventaire.dir_2024 d ON ST_Intersects(d.geom, p1.geom);
*/
SELECT p.npp, id_point, id_ech, pt.formation, x, y, p.depn AS dep, d.ex AS DT
FROM points_tir_final pt
INNER JOIN points p USING (id_point)
INNER JOIN point p1 USING (id_point)
...
/*
SELECT count(*) FROM point_ech WHERE id_ech = 144;
SELECT count(*) FROM point_lt WHERE id_ech = 144;

SELECT pe.id_ech, pe.id_point, p.npp, pe.dep
FROM point_ech pe
INNER JOIN point p USING (id_point)
INNER JOIN point_lt pl USING (id_ech, id_point)
LEFT JOIN points_tir_final pt USING (id_ech, id_point)
WHERE pe.id_ech = 144
AND pt.id_ech IS NULL
ORDER BY dep, npp;

SELECT pt.id_ech, pt.id_point, ps.npp, pe.dep
FROM points_tir_final pt
INNER JOIN points ps USING (id_point)
INNER JOIN point_ech pe ON ps.id_ech_ph1 = pe.id_ech AND ps.id_point = pe.id_point
LEFT JOIN point_lt pl ON pt.id_ech = pl.id_ech AND pt.id_point = pl.id_point
WHERE pl.id_ech IS NULL
ORDER BY dep, npp;
*/

DELETE FROM hydro WHERE id_ech = 144;
DELETE FROM point_lt WHERE id_ech = 144;
DELETE FROM point_ech WHERE id_ech = 144;
...
--INSERT INTO point_lt (id_ech, id_point, formation, azpoint, decli_pt, echelon_init)
SELECT pt.id_ech, pt.id_point, pt.formation, t.aztrans, t.decli, p.npp
, d.ex AS echelon_init
FROM public.points_tir_final pt
INNER JOIN point p USING (id_point)
INNER JOIN transect t USING (id_transect)
...
-- Contrôle echelon_init
SELECT id_ech, id_point
FROM point_lt
WHERE echelon_init IS NULL AND id_ech IN (144, 145);
...
-- Contrôle secteurs_cn
SELECT id_ech, id_point
FROM point_lt
WHERE secteur_cn IS NULL AND id_ech IN (144, 145);
