...
WITH en_base AS (
    ...
    , int4range(min(annee), CASE WHEN max(annee) >= 2023 THEN NULL ELSE max(annee) END, '[]') AS validite_base
    FROM donnees_visu
    GROUP BY donnee
)
, en_meta AS (
