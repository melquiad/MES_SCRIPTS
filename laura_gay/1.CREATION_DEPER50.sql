...
--VALUES ('U_DEPER', 'AUTRE', 'NOMINAL', 'Intensité du dépérissement', 'Caractérise l''intensité du dépérissement de chaque placette en 6 classes (de 0 à 50% et plus)');

-- 2.Documentation des modalités
--INSERT INTO metaifn.abmode (unite, mode, position, classe, etendue, libelle, definition)
--VALUES ('U_DEPER', '0', 1, 1, 1, '0%', 'Aucun arbre mort ou arbre avec une mortalité de branches supérieure à 50 %')
--, ('U_DEPER', '1', 2, 2, 1, 'moins de 20%', 'Moins de 20 % (exclu) d''arbres morts ou arbres avec une mortalité de branches supérieure à 50 %')
--, ('U_DEPER', '2', 3, 3, 1, 'entre 20 et 30%', 'Entre 20 % (inclus) et 30 % (exclu) d''arbres morts ou arbres avec une mortalité de branches supérieure à 50 %')
--, ('U_DEPER', '3', 4, 4, 1, 'entre 30 et 40%', 'Entre 30 % (inclus) et 40 % (exclu) d''arbres morts ou arbres avec une mortalité de branches supérieure à 50 %')
--, ('U_DEPER', '4', 5, 5, 1, 'entre 40 et 50%', 'Entre 40 % (inclus) et 50 % (exclu) d''arbres morts ou arbres avec une mortalité de branches supérieure à 50 %')
--, ('U_DEPER', '5', 6, 6, 1, '50% et plus', 'Au moins 50 % (inclus) d''arbres morts ou arbres avec une mortalité de branches supérieure à 50 %');

-- 3.Documentation de la donnée
SELECT * FROM metaifn.ajoutdonnee('DEPER50', NULL, 'U_DEPER', 'AUTRE', NULL, 6, 'varchar(1)', 'CC', TRUE, TRUE, 
...
SELECT * 
FROM metaifn.ajoutchamp('DEPER50', 'U_G3FORET', 'INV_EXP_NM', FALSE, 0, 17, 'varchar', 1);

SELECT * 
FROM metaifn.ajoutchamp('DEPER50', 'U_P3POINT', 'INV_EXP_NM', FALSE, 0, 17, 'varchar', 1);
...
COMMENT ON COLUMN inv_exp_nm.u_g3foret.DEPER50 IS 'Intensité du dépérissement' ;
...
WHERE INCREF BETWEEN 12 AND 17
		--AND DEPER50 IS NOT NULL
ORDER BY INCREF, NPP
