...
-- 1.Création et documentation de l’unité DEPER
...
SELECT * FROM metaifn.ajoutdonnee('DEPER25', NULL, 'DEPER', 'AUTRE', NULL, 6, 'varchar(1)', 'CC', TRUE, TRUE, 
'Intensité du dépérissement', 
'Caractérise l''intensité du dépérissement de chaque placette en 6 classes (de 0 à 50% et plus)');

--DELETE FROM addonnee WHERE donnee = 'U_DEPER25';

-- 4.Documentation de la colonne en base
SELECT * 
FROM metaifn.ajoutchamp('DEPER25', 'G3FORET', 'INV_EXP_NM', FALSE, 0, 17, 'varchar', 1, 1, FALSE,'lhaugomat');

SELECT * 
FROM metaifn.ajoutchamp('DEPER25', 'P3POINT', 'INV_EXP_NM', FALSE, 0, 17, 'varchar', 1, 1, FALSE,'lhaugomat');

UPDATE metaifn.afchamp 
SET calcin = 12, calcout = 17, validin = 12, validout = 17, defin = 12, defout = NULL  
WHERE famille = 'INV_EXP_NM' AND donnee = 'DEPER25';
...
ADD COLUMN DEPER25 character(1); 
COMMENT ON COLUMN inv_exp_nm.g3foret.DEPER25 IS 'Intensité du dépérissement' ;
...
ADD COLUMN DEPER25 character(1); 
COMMENT ON COLUMN inv_exp_nm.p3point.DEPER25 IS 'Intensité du dépérissement' ;
