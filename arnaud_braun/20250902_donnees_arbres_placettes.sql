...
WHERE annee = 2024 AND csa IN ('1','3','5')
UNION
SELECT p.idp, plt.datepoint, r.csa, pe.ser_86, r2.tform, gf.comp_r
, d.tplant, pl.tpespar1, pl.tpespar2, d.href_dm AS hauteur_de_ref_en_dm, d.sver, dm.tcat10
, e.pent2, e.topo, e.expo, e.masque, e2.msud, e.humus, e.tsol
...
-------------------------------------
SELECT p.idp, p.id_point, v1.annee, a.a , a.c13_mm / 10::numeric AS c13_en_cm, a1.veget5, am.espar, a2.libelle
, ROUND(ST_X(p.geom)::NUMERIC) AS xl93, ROUND(ST_Y(p.geom)::NUMERIC) AS yl93
FROM v_liste_points_lt2 v1
...
INNER JOIN arbre_m1 am USING (id_point, a) --> pour récupérer l'essence de la v1
INNER JOIN metaifn.abmode a2 ON am.espar = a2."mode" AND a2.unite = 'ESPAR1'
WHERE annee = 2024
ORDER BY annee, idp, a;

-------------------------------------------------------------------------------------------
-- Données arbres de 2ème visite nouvellement recensables ( les arbres 1ère visite nouvellement
-- recensables (mesurés dans lt1pi2) sont déjà inclus dans la requête arbres 1ère visite au-dessus)
-------------------------------------------------------------------------------------------
...
INNER JOIN inv_exp_nm.g3essence ge ON v1.npp = ge.npp
WHERE annee BETWEEN 2020 AND 2024 AND csa IN ('1','3','5')
UNION
SELECT v1.annee, p.idp, r.csa, pe.espar, pe.tca, pe.tcl
FROM v_liste_points_lt2 v1
INNER JOIN point p  USING (id_point)
