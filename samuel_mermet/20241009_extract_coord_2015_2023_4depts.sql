...
SELECT vlpl.npp, p.idp, r.csa, pe.dep, vlpl.annee
...
ORDER BY 4,1;
	...
	-- demande complémentaire du 24/03/2025 avec deppr et posipr donc sans les points LT2
SELECT vlpl.npp, p.idp, r.csa, pe.dep, vlpl.annee, d.deppr, pm.posipr
, round(st_x(p.geom)::NUMERIC) AS xl93,  round(st_y(p.geom)::NUMERIC) AS yl93
FROM v_liste_points_lt1 vlpl 
INNER JOIN point p USING (id_point)
INNER JOIN point_ech pe USING (id_ech, id_point)
INNER JOIN reconnaissance r USING (id_ech, id_point)
LEFT JOIN descript_m1 d USING (id_ech, id_point)
LEFT JOIN point_m1 pm USING (id_ech, id_point)
WHERE pe.dep IN ('06','48','83','88','25') AND vlpl.annee BETWEEN 2015 AND 2023
UNION
SELECT vlpl.npp, p.idp, r.csa, pe.dep, vlpl.annee, d.deppr, pm.posipr
, round(st_x(p.geom)::NUMERIC) AS xl93,  round(st_y(p.geom)::NUMERIC) AS yl93
