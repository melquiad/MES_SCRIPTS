...
WHERE a.npp = e.npp AND a.incref = 1;
...
FROM inv_exp_nm.e2point e
WHERE a.npp = e.npp
AND a.incref BETWEEN 14 AND 15;
--AND NOT e.u_inv_facon;
...
SELECT * FROM metaifn.ajoutchamp('MORTBG2'::varchar, 'G3ARBRE'::varchar, 'INV_EXP_NM'::varchar, FALSE::boolean, 1, NULL::int4, 'bpchar'::varchar, 1::int4);
SELECT * FROM metaifn.ajoutchamp('MORTBG2', 'P3ARBRE', 'INV_EXP_NM', FALSE, 2, NULL, 'bpchar', 1);

UPDATE metaifn.afchamp
SET calcin = 1,calcout = 17, validin = 1, validout = 16
WHERE famille ~~* 'inv_exp_nm'
AND format ~~* 'tg3arbre'
AND donnee ~~* 'MORTBG2';

UPDATE metaifn.afchamp
SET calcin = 2,calcout = 17, validin = 2, validout = 16
WHERE famille ~~* 'inv_exp_nm'
AND format ~~* 'tp3arbre'
AND donnee ~~* 'MORTBG2';
