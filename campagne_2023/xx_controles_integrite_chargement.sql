...
WHERE right(npp, 1) = 'T'
AND numvisi = '1'
AND anref = 2023; -- => 7628 points

-- En base de production
SELECT count(*) AS nb_points
FROM v_liste_points_lt1
WHERE annee = 2023; -- => 7628 points


-- DÉCOMPTE DES EFFECTIFS PAR DONNÉES DE RECONNAISSANCE
-- Dans Soif
SELECT count(*) AS nb_lignes, count(auteurlt) AS auteurlt1, count(auteurlt_2) AS auteurlt2, count(auteurlt_3) AS auteurlt3, count(datepoint) AS datepoint, count(reco) AS reco
...
-- En base de production
SELECT count(*) AS nb_lignes, count(a1.matricule) AS auteurlt1, count(a2.matricule) AS auteurlt2, count(a3.matricule) AS auteurlt3, count(datepoint) AS datepoint, count(reco) AS reco
, count(duracc) AS duracc, count(posipr) AS posipr, count(pclos) AS pclos, count(pdiff) AS pdiff, count(csa) AS csa, count(obscsa) AS obscsa, count(utip) AS utip
, count(bois) AS bois, count(doute_bois) AS doute_bois, count(autut) AS autut, count(tform) AS tform, count(tauf) AS tauf, count(leve) AS leve
, count(qleve) AS qleve, count(eflt) AS eflt, count(rp) AS rp, count(azrp_gd) AS azrp_gd, count(drp_cm) AS drp_cm
, count(qreco) AS qreco, count(qbois) AS qbois, count(pl.suppl->>'pobs') AS pobs, count(vegrp) AS vegrp, count(esprp) AS esprp, count(c13rp_mm) AS c13rp_mm
FROM v_liste_points_lt1
...
LEFT JOIN agent_lt a1 ON pl.id_ech = a1.id_ech AND pl.id_point = a1.id_point AND a1.num_auteurlt = 1
LEFT JOIN agent_lt a2 ON pl.id_ech = a2.id_ech AND pl.id_point = a2.id_point AND a2.num_auteurlt = 2
LEFT JOIN agent_lt a3 ON pl.id_ech = a3.id_ech AND pl.id_point = a3.id_point AND a3.num_auteurlt = 3
WHERE annee = 2023;
...
, count(incid) AS incid, count(dc) AS dc, count(dcespar1) AS dcespar1, count(andain) AS andain, count(tplant) AS tplant, count(entp) AS entp
...
, count(dispnr) AS dispnr, count(fouil) AS fouil, count(ornr) AS ornr, count(predom) AS predom, count(prnr) AS prnr, count(tcnr) AS tcnr
, count(atpyr) AS atpyr, count(ncbuis10) AS ncbuis10, count(ncbuis_a) AS ncbuis_a, count(ncbuis_b) AS ncbuis_b, count(ncbuis_c) AS ncbuis_c, count(ncbuis_d) AS ncbuis_d, count(ncbuis_e) AS ncbuis_e, count(azdbuis_gd) AS zdbuis_gd
FROM soif.v1e3point
INNER JOIN soif.point_anref USING (npp)
LEFT JOIN soif.v1e3nrpoint USING (npp)
WHERE right(npp, 1) = 'T'
AND numvisi = '1'
AND anref = 2023;
...
, count(incid) AS incid, count(dc) AS dc, count(dcespar1) AS dcespar1, count(andain) AS andain, count(tplant) AS tplant
...
-- DÉCOMPTE DES EFFECTIFS D'INFOS PLACETTES DE RENOUVELLEMENT
-- Dans Soif
SELECT count(libnr_sp1) AS libnr_sp1, count(pint_sp1) AS pint_sp1
, count(libnr_sp2) AS libnr_sp2, count(pint_sp2) AS pint_sp2
FROM soif.v1e3nrpoint
INNER JOIN soif.point_anref USING (npp)
WHERE right(npp, 1) = 'T'
AND numvisi = '1'
AND anref = 2023;
...
-- En base de production
SELECT count(libnr_sp) FILTER (WHERE nsnr = 1) AS libnr_sp1, count(pint_sp) FILTER (WHERE nsnr = 1) AS pint_sp1
, count(libnr_sp) FILTER (WHERE nsnr = 2) AS libnr_sp2, count(pint_sp) FILTER (WHERE nsnr = 2) AS pint_sp2
FROM v_liste_points_lt1
INNER JOIN renouv USING (id_ech, id_point)
WHERE annee = 2023;

-- DÉCOMPTE DES EFFECTIFS D'ESPÈCES DE RENOUVELLEMENT
-- Dans Soif
SELECT count(npp) AS nb_lignes, count(nsnr) AS nsnr, count(chnr) AS chnr, count(espar) AS espar, count(nbint) AS nint, count(nbrou) AS nbrou, count(nfrot) AS nfrot, count(nmixt) AS nmixt
FROM soif.v1e3nrspot
INNER JOIN soif.point_anref USING (npp)
WHERE right(npp, 1) = 'T'
AND numvisi = '1'
AND anref = 2023;
...
-- En base de production
SELECT count(npp) AS nb_lignes, count(nsnr) AS nsnr, count(chnr) AS chnr, count(espar) AS espar, count(nint) AS nint, count(nbrou) AS nbrou, count(nfrot) AS nfrot, count(nmixt) AS nmixt
FROM v_liste_points_lt1
INNER JOIN espar_renouv USING (id_ech, id_point)
WHERE annee = 2023;
...
, count(c13_inf_mm) AS c13_inf_mm, count(c13_sup_mm) AS c13_sup_mm
FROM soif.v1e3arbre
INNER JOIN soif.point_anref USING (npp)
WHERE right(npp, 1) = 'T'
AND numvisi = '1'
...
, count(htot_dm) AS htot_dm, count(decoupe) AS decoupe, count(hdec_dm) AS hdec_dm, count(ddec_cm) AS ddec_cm, count(c13_mm) AS c13_mm, count(qbp) AS qbp, count(hbv_dm) AS hbv_dm, count(hbm_dm) AS hbm_dm, count(hrb_dm) AS hrb_dm
, count(ma) AS ma, count(mr) AS mr, count(hcd_cm) AS hcd_cm
, count(arbre.suppl->>'c13_inf_mm') AS c13_inf_mm, count(arbre.suppl->>'c13_sup_mm') AS c13_sup_mm
FROM v_liste_points_lt1
...
FROM fla_lt f
INNER JOIN echantillon e ON f.id_ech = e.id_ech AND e.type_ech = 'IFN' AND e.type_ue = 'T' AND e.phase_stat = 2
...
FROM soif.v2e4point
INNER JOIN soif.point_anref USING (npp)
INNER JOIN soif.e1point USING (npp)
WHERE numvisi = '2'
AND inv = 'T'
AND anref = 2023; -- => 6033 points

-- En base de production
SELECT count(*) AS nb_points
FROM v_liste_points_lt2
WHERE annee = 2023; -- => 6033 points
...
-- Dans Soif
SELECT count(*) AS nb_lignes, count(auteurlt5) AS auteurlt5, count(auteurlt5_2) AS auteurlt5_2, count(auteurlt5_3) AS auteurlt5_3, count(datepoint5) AS datepoint5, count(reco5) AS reco5, count(pointok5) AS pointok5, count(csa5) AS csa5, count(evo_csa5) AS evo_csa5
...
-- En base de production
SELECT count(*) AS nb_lignes, count(a1.matricule) AS auteurlt5, count(a2.matricule) AS auteurlt5_2, count(a3.matricule) AS auteurlt5_3, count(datepoint) AS datepoint5, count(reco) AS reco5, count(pointok5) AS pointok5, count(csa) AS csa5, count(evo_csa) AS evo_csa5
...
, count(def5) AS def5, count(evo_utip) AS evo_utip5, count(eflt) AS eflt5, count(qreco) AS qreco5, count(qbois) AS qbois5, count(pl.suppl->>'pobs') AS pobs5
FROM v_liste_points_lt2
INNER JOIN point_lt pl USING (id_ech, id_point)
LEFT JOIN reconnaissance USING (id_ech, id_point)
...
INNER JOIN soif.e1point USING (npp)
INNER JOIN soif.point_anref USING (npp)
LEFT JOIN soif.v2e5nrpoint USING (npp)
WHERE numvisi = '2'
AND inv = 'T'
AND anref = 2023;
...
-- DÉCOMPTE DES EFFECTIFS D'INFOS PLACETTES DE RENOUVELLEMENT V1
-- Dans Soif
SELECT count(libnr_sp1) AS libnr_sp1, count(pint_sp1) AS pint_sp1
, count(libnr_sp2) AS libnr_sp2, count(pint_sp2) AS pint_sp2
FROM soif.v2e5nrpoint
INNER JOIN soif.e1point USING (npp)
INNER JOIN soif.point_anref USING (npp)
WHERE numvisi = '2'
AND inv = 'T'
...
-- DÉCOMPTE DES EFFECTIFS D'ESPÈCES DE RENOUVELLEMENT V1
-- Dans Soif
SELECT count(npp) AS nb_lignes, count(nsnr) AS nsnr, count(chnr) AS chnr, count(espar) AS espar, count(nbint) AS nint, count(nbrou) AS nbrou, count(nfrot) AS nfrot, count(nmixt) AS nmixt
FROM soif.v2e5nrspot
INNER JOIN soif.e1point USING (npp)
INNER JOIN soif.point_anref USING (npp)
WHERE numvisi = '2'
AND inv = 'T'
...
FROM (
    ...
    INNER JOIN soif.e1point e ON d.npp = e.npp
    WHERE numvisi = '2'
    AND inv = 'T'
    AND anref = 2023
    AND donnee = $$DATEARBRE$$
...
INNER JOIN arbre USING (id_ech, id_point)
LEFT JOIN arbre_2014 USING (id_ech, id_point, a)
LEFT JOIN arbre_m1 USING (id_ech, id_point, a)
LEFT JOIN arbre_m1_2014 USING (id_ech, id_point, a)
LEFT JOIN sante USING (id_ech, id_point, a)
WHERE annee = 2023;
...
WHERE right(npp, 1) = 'R'
AND numvisi = '1'
AND anref = 2023;-- => 155 points

-- En base de production
SELECT count(*) AS nb_points
FROM v_liste_points_lt1_pi2
WHERE annee = 2023; -- => 155 points
...
SELECT * FROM (
    ...
    , CASE 
        ...
        WHEN leve = '0' AND jsonb_path_query_first(r.qual_data, ('$[*] ? (@.donnee == "QLEVE")."qdonnee"')::jsonpath)->>0 IS NULL THEN '99h. Observation sur QLEVE absente'
...
FROM v_liste_points_lt1 v
INNER JOIN point_lt USING (id_ech, id_point)
WHERE (datepoint < '01-10-2022'::DATE
	OR datepoint >= '21-12-2023'::DATE)
AND v.annee = 2023;
...
INNER JOIN reco_2015 rm USING (id_ech, id_point)
...
SELECT * FROM (
    ...
    , CASE 
        ...
        WHEN pbuis != '0' AND atpyr IS NULL THEN '41a. ATPYR NULL'
        WHEN COALESCE(pbuis, '0') = '0' AND atpyr IS NOT NULL THEN '41b. ATPYR devrait être à NULL'
        WHEN atpyr = '1' AND ncbuis_a IS NULL THEN '42a. NCBUIS_A NULL'
        WHEN COALESCE(atpyr, '0') = '0' AND ncbuis_a IS NOT NULL THEN '42b. NCBUIS_A devrait être à NULL'
        WHEN atpyr = '1' AND ncbuis_b IS NULL THEN '42c. NCBUIS_B NULL'
        WHEN COALESCE(atpyr, '0') = '0' AND ncbuis_b IS NOT NULL THEN '42d. NCBUIS_B devrait être à NULL'
        WHEN atpyr = '1' AND ncbuis_c IS NULL THEN '42e. NCBUIS_C NULL'
        WHEN COALESCE(atpyr, '0') = '0' AND ncbuis_c IS NOT NULL THEN '42f. NCBUIS_C devrait être à NULL'
        WHEN atpyr = '1' AND ncbuis_d IS NULL THEN '42g. NCBUIS_D NULL'
        WHEN COALESCE(atpyr, '0') = '0' AND ncbuis_d IS NOT NULL THEN '42h. NCBUIS_D devrait être à NULL'
        WHEN atpyr = '1' AND ncbuis_e IS NULL THEN '42i. NCBUIS_E NULL'
        WHEN COALESCE(atpyr, '0') = '0' AND ncbuis_e IS NOT NULL THEN '42j. NCBUIS_E devrait être à NULL'
        WHEN ncbuis_a + ncbuis_b + ncbuis_c + ncbuis_d + ncbuis_e > 10 THEN '43. Somme des NCBUIS > 10'
        WHEN ncbuis_a + ncbuis_b + ncbuis_c + ncbuis_d + ncbuis_e = 10 AND ncbuis10 IS NULL THEN '44a. NCBUIS10 NULL'
        WHEN ncbuis_a + ncbuis_b + ncbuis_c + ncbuis_d + ncbuis_e != 10 AND ncbuis10 IS NOT NULL THEN '44b. NCBUIS10 devrait être à NULL'
        WHEN ncbuis10 = '1' AND azdbuis_gd IS NULL THEN '45a. AZDBUIS NULL'
        WHEN COALESCE(ncbuis10, '0') = '0' AND azdbuis_gd IS NOT NULL THEN '45b. AZDBUIS devrait être à NULL'
        WHEN tcnr IS NULL THEN '46. TCNR NULL'
        WHEN tcnr != 'X' AND ornr IS NULL THEN '47a. ORNR NULL'
        WHEN COALESCE(tcnr, 'X') = 'X' AND ornr IS NOT NULL THEN '47b. ORNR devrait être à NULL'
        WHEN tcnr != 'X' AND prnr IS NULL THEN '48a. PRNR NULL'
        WHEN COALESCE(tcnr, 'X') = 'X' AND prnr IS NOT NULL THEN '48b. PRNR devrait être à NULL'
        WHEN tcnr != 'X' AND fouil IS NULL THEN '49a. FOUIL NULL'
        WHEN COALESCE(tcnr, 'X') = 'X' AND fouil IS NOT NULL THEN '49b. fouil devrait être à NULL'
        WHEN tcnr != 'X' AND predom IS NULL THEN '50a. PREDOM NULL'
        WHEN COALESCE(tcnr, 'X') = 'X' AND predom IS NOT NULL THEN '50b. PREDOM devrait être à NULL'
        WHEN tcnr != 'X' AND dispnr IS NULL THEN '51a. DISPNR NULL'
        WHEN COALESCE(tcnr, 'X') = 'X' AND dispnr IS NOT NULL THEN '51b. DISPNR devrait être à NULL'
      END AS erreur
    FROM v_liste_points_lt1 v
    INNER JOIN reconnaissance r USING (id_ech, id_point)
    INNER JOIN description d USING (id_ech, id_point)
...
SELECT npp, id_ech, id_point, csa, peupnr, href_dm, dc, bord, sver, tcat10
FROM v_liste_points_lt1 v
INNER JOIN reconnaissance r USING (id_ech, id_point)
INNER JOIN description d USING (id_ech, id_point)
...
-- CONTROLES DES DONNEES RPPGO
SELECT * FROM (
    SELECT v.npp, v.id_ech, v.id_point--, *
    , CASE 
        WHEN dispnr IN ('1', '3') AND r1.id_ech IS NULL THEN '01. Sous-placette 1 absente'
        WHEN dispnr IN ('2', '3') AND r2.id_ech IS NULL THEN '02. Sous-placette 2 absente'
        WHEN r1.id_ech IS NOT NULL AND r1.libnr_sp IS NULL THEN '03. LIBNR_SP1 NULL'
        WHEN r2.id_ech IS NOT NULL AND r2.libnr_sp IS NULL THEN '04. LIBNR_SP2 NULL'
      END AS erreur
    FROM v_liste_points_lt1 v
    INNER JOIN reconnaissance r USING (id_ech, id_point)
    INNER JOIN descript_m1 d1 USING (id_ech, id_point)
    LEFT JOIN renouv r1 ON d1.id_ech = r1.id_ech AND d1.id_point = r1.id_point AND r1.nsnr = 1
    LEFT JOIN renouv r2 ON d1.id_ech = r2.id_ech AND d1.id_point = r2.id_point AND r2.nsnr = 2
    WHERE v.annee = 2023
) AS t
WHERE erreur IS NOT NULL
ORDER BY erreur, npp;
...
SELECT * FROM (
    ...
    , CASE 
        ...
        WHEN mes_c13 = '2' AND a.suppl->>'c13_inf_mm' IS NULL THEN '12d. C13_INF_MM NULL'
        WHEN COALESCE(mes_c13, '1') != '2' AND a.suppl->>'c13_inf_mm' IS NOT NULL THEN '12e. C13_INF_MM devrait être à NULL'
        WHEN mes_c13 = '2' AND a.suppl->>'c13_sup_mm' IS NULL THEN '12f. C13_SUP_MM NULL'
        WHEN COALESCE(mes_c13, '1') != '2' AND a.suppl->>'c13_sup_mm' IS NOT NULL THEN '12g. C13_SUP_MM devrait être à NULL'
--        WHEN a.suppl->>'c13_sup_mm' > a.suppl->>'c13_inf_mm' THEN '12h. C13_SUP_MM > C13_INF_MM'
        ...
        WHEN veget = '0' AND espar > '50' AND c13_mm >= 705 AND mortb IN ('0', '1', '2', '3') AND ma IS NULL THEN '17. MA NULL'
...
GROUP BY 1
ORDER BY 1;
-- Tous à NCERN = 0 ou NULL donc OK.
...
ORDER BY npp, am.a;        -- 6 erreurs sur l'espèce
...
FROM dist_arbat
WHERE dist > 2
ORDER BY npp, a;   -- 18 incohérences sur les distances entre arbres attachés
...
SELECT * FROM (
    ...
    , CASE 
        ...
        WHEN msud IS NULL THEN '05. MSUD NULL'
...
WHERE erreur IS NOT NULL
ORDER BY erreur;
-- commentaires absents sur LIGNERIV et les QUALHAB = X
...
WHERE rang_ecart = 1
ORDER BY npp, a;
-- 7 arbres présentent des problèmes de numérotation
...
WITH t AS (
    ...
    , CASE
        ...
        WHEN pointok5 != '0' AND rm5.csa IN ('1', '3', '5') AND def5 IS NULL THEN '05a. DEF5 NULL'
        WHEN pointok5 != '0' AND rm5.csa IN ('1', '3', '5') AND r.csa IN ('1', '3', '5') AND r1.utip = 'X' AND def5 NOT IN ('0', '1', '2') THEN '05b. Incohérence entre CSA boisé, CSA5 boisé et DEF5'
        WHEN pointok5 != '0' AND rm5.csa IN ('1', '3', '5') AND r.csa NOT IN ('1', '3', '5') AND evo_csa = '1' AND def5 NOT IN ('3', '4', '5') THEN '05c. Incohérence entre CSA boisé, CSA5 non boisé et DEF5'
        WHEN pointok5 != '0' AND rm5.csa IN ('1', '3', '5') AND r.csa IN ('1', '3', '5') AND r1m5.utip = 'X' AND r1.utip != 'X' AND r2.evo_utip = '1' AND r2.def5 NOT IN ('3', '4', '5') THEN '05c. Incohérence entre CSA boisé, CSA5 boisé, UTIP autre, UTIP5 non autre, EVO_UTIP rapide et DEF5'
...
ORDER BY erreur, npp;
-- 1 erreur sur NPP = 18-03-196-1-152T

/*
SELECT v.npp, v.id_ech, v.id_point, rm5.csa AS csa, r.csa AS csa5, evo_csa, r2.qual_data, r2.def5, p.reco, pm.pointok5
FROM v_liste_points_lt2 v
INNER JOIN point_lt p USING (id_ech, id_point)
LEFT JOIN point_m2 pm USING (id_ech, id_point)
LEFT JOIN reconnaissance r USING (id_ech, id_point)
...
WHERE t.erreur IS NOT NULL
ORDER BY erreur, npp;
-- => manque de commentaires seulement

/*
SELECT instp5, bplant_dm, iplant_dm
FROM v_liste_points_lt2 v
INNER JOIN descript_m2 dm2 USING (id_ech, id_point)
INNER JOIN plantations pl USING (id_ech, id_point)
WHERE v.annee = 2023
AND instp5 = 'P'
AND bplant_dm IS NULL;
*/
...
INNER JOIN arbre_m2 am2 USING (id_ech, id_point, a)
INNER JOIN arbre_m1 am1 ON am2.id_ech > am1.id_ech AND am2.id_point = am1.id_point AND am2.a = am1.a
WHERE (npp, a.a) IN (('18-48-240-1-217T', 2 ), ('18-49-097-1-154T', 3 ), ('18-51-204-1-049T', 3 ), ('18-01-270-1-143T', 10), 
('18-23-189-1-170T', 3 ), ('18-41-152-1-129T', 8 ), ('18-44-078-1-153T', 11), ('18-54-243-1-050T', 4 ), 
('18-79-115-1-158T', 1 ))
ORDER BY npp, a;
*/
...
FROM v_liste_points_lt1_pi2 v
INNER JOIN point_lt USING (id_ech, id_point)
WHERE (datepoint < '16-10-2022'::DATE
    OR datepoint >= '20-12-2023'::DATE)
AND v.annee = 2023;
-- pas d'erreur
...
ORDER BY npp;       -- => 1 erreur qu'ON laisse passer (point 2è passage)
...
WHERE erreur IS NOT NULL
ORDER BY erreur;
-- commentaires absents sur QUALHAB1 et LIGNERIV
