...
SELECT n.nppg, g.usite, g.site, '5' AS cyc, g.inv, m.absc, m.ord, n.tirmax, n.incref, ne.zp, ne.depn AS depp, ROUND(ST_X(ST_Transform(n.geom, 932006))::NUMERIC) AS xlt, ROUND(ST_Y(ST_Transform(n.geom, 932006))::NUMERIC) AS ylt
, ROUND(ST_X(ST_Transform(n.geom, 932006))::NUMERIC) AS xlp, ROUND(ST_Y(ST_Transform(n.geom, 932006))::NUMERIC) AS ylp, ne.regn, ST_SetSRID(n.geom, 910001) AS geom
FROM echantillon e
...
SELECT p.npp, n.nppg, p.code_pt, ROUND(ST_X(ST_Transform(m.geom, 932006))::NUMERIC) AS xl, ROUND(ST_Y(ST_Transform(m.geom, 932006))::NUMERIC) AS yl, auteurpi, datepi, occ, cso, poids, dep, pe.zp
, vp.annee - 2005 AS incref, id_unite_deb + row_number() OVER(ORDER BY p.npp) AS id_unite, p.idp
, ST_SetSRID(p.geom, 910001) AS geom, '5' AS cyc
, dbpi, pbpi, uspi, ufpi, tfpi, phpi, blpi
...
/*
-- Vérification d'absence de doublons d'ID_UNITE dans E1POINT
...
-- e2point
INSERT INTO inv_exp_nm.e2point (npp, incref, dep, cyc, auteurlt, datepoint, csa, formation, poids, obscsa, id_unite, utip, bois, autut, tauf, tform, ser_86, ser_alluv, greco, regn, eflt, qleve, qbois)
...
, left(pe.ser_86, 1) AS greco, pe.regn, r1.eflt, r1.qleve, r1.qbois
FROM point_ech pe 
...
-- g3foret
INSERT INTO inv_exp_nm.g3foret (npp, incref, dep, cyc
    , plisi, cslisi, dist, dc, tplant, gest, integr, iti, portance, asperite, pentexp, incid
   , andain
    , bord, dcespar1, peupnr, sver, nincid, orniere
    , caracthab
    , pbuis, atpyr, ncbuis10, ncbuis_a, ncbuis_b, ncbuis_c, ncbuis_d, ncbuis_e)
...
, c.dcespar1, d.peupnr, d.sver, d.nincid, d1.orniere, d.caracthab, b.pbuis, b.atpyr, b.ncbuis10, b.ncbuis_a, b.ncbuis_b, b.ncbuis_c, b.ncbuis_d, b.ncbuis_e  
FROM v_liste_points_lt1 vp
...
, atpyr, ncbuis10, ncbuis_a, ncbuis_b, ncbuis_c, ncbuis_d, ncbuis_e)
...
b.atpyr, b.ncbuis10, b.ncbuis_a, b.ncbuis_b, b.ncbuis_c, b.ncbuis_d, b.ncbuis_e
FROM v_liste_points_lt1 vp
INNER JOIN reconnaissance r USING (id_ech, id_point)
INNER JOIN point_ech pe USING (id_ech, id_point)
INNER JOIN descript_m1 d1 USING (id_ech, id_point)
...
-- g3agedom
INSERT INTO inv_exp_nm.g3agedom (npp, incref, dep, cyc
	, su, numa, a, typdom, age13, cam, ncerncar, longcar, sfcoeur)
...
-- g3arbre
INSERT INTO inv_exp_nm.g3arbre (npp, incref, dep, cyc
  , a, ori, veget, acci, lib, qbp, hbv, hbm, espar, decoupe, simplif, cible, arbat, ncern, deggib
  , hdec, htot, ir5, irn, c13
...
WITH echs AS (
    ...
    FROM echantillon ep
    INNER JOIN campagne c USING (id_campagne)
    INNER JOIN echantillon et ON ep.id_campagne = et.id_campagne AND et.type_ech = 'T' AND et.phase_stat = 1
    WHERE ep.type_ech = 'P'
    AND ep.phase_stat = 1
...
, c.dcespar1, d.peupnr, d.nincid, d1.orniere, b.pbuis, b.atpyr, b.ncbuis10, b.ncbuis_a, b.ncbuis_b, b.ncbuis_c, b.ncbuis_d, b.ncbuis_e
FROM v_liste_points_lt1 vp
INNER JOIN reconnaissance r USING (id_ech, id_point)
INNER JOIN point_ech pe USING (id_ech, id_point)
INNER JOIN description d USING (id_ech, id_point)
...
UPDATE inv_exp_nm.g3morts SET veget5 = 'M' WHERE incref = 12 AND veget IN ('5', 'C') AND veget5 = '0';
UPDATE inv_exp_nm.g3morts SET veget5 = '1' WHERE incref = 12 AND veget = 'A' AND veget5 = '0';
UPDATE inv_exp_nm.g3morts SET veget5 = '2' WHERE incref = 12 AND veget = 'A' AND veget5 = 'M';
...
INSERT INTO prod_exp.g3arbre5 (npp, a, veget, dpr, azpr, c13, htot, espar, decoupe, hdec)
SELECT vp2.npp, a.a, a1.veget, ROUND((a1.dpr_cm / 100.0)::NUMERIC, 2) AS dpr, a1.azpr_gd * PI() / 200 AS azpr, a.c13_mm / 1000.0 AS c13, a1.htot_dm / 10.0 AS htot, a1.espar, a1.decoupe, a1.hdec_dm / 10.0 AS hdec
...
INNER JOIN reconnaissance r ON vp1.id_ech = r.id_ech AND vp1.id_point = r.id_point AND r.csa IN ('1', '2', '3')
WHERE vp2.annee = 2023;

INSERT INTO prod_exp.p3arbre5 (npp, a, veget, dpr, azpr, c13, htot, espar, decoupe, hdec)
SELECT vp2.npp, a.a, a1.veget, ROUND((a1.dpr_cm / 100.0)::NUMERIC, 2) AS dpr, a1.azpr_gd * PI() / 200 AS azpr, a.c13_mm / 1000.0 AS c13, a1.htot_dm / 10.0 AS htot, a1.espar, a1.decoupe, a1.hdec_dm / 10.0 AS hdec
FROM v_liste_points_lt2 vp2
INNER JOIN descript_m2 USING (id_ech, id_point)
INNER JOIN arbre a USING (id_ech, id_point)
...
WHERE et2.type_ech = 'P'
AND et2.ech_parent IS NOT NULL
AND et2.nom_ech LIKE 'FR%'
ORDER BY et2.id_ech;

-- liste des arbres levés en deuxième visite et absents au premier lever
